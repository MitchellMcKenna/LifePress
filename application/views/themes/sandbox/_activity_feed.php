            ...
            <?php foreach ($items as $item): ?>
                ...
                <li class="item <?php echo $item->get_feed_class()?>">
                    <?php if ($item->get_feed_domain() == $this->config->item('base_url')): ?>
                        <p class="site_info" style="background: transparent url(<?php echo $item->get_feed_icon()?>) 0 center no-repeat"><?php echo $this->lang->line('posted_a'); ?> <a href="<?php echo $this->config->item('base_url')?>items/site/lifepress">blog entry</a></p>
                    <?php else: ?>
